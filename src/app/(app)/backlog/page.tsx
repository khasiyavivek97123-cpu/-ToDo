...
import { Layers, ChevronDown, ChevronRight, Calendar } from "lucide-react";
...
export default function BacklogPage() {
  ...
  const backlogTasks = tasks.filter((t) => !t.dueDate);
  const activeTasks = backlogTasks.filter((t) => !t.completed);
  const completedTasks = backlogTasks.filter((t) => t.completed);
      ...
      <div className="space-y-2">
        {activeTasks.length > 0 ? (
          ...
          <EmptyState
            ...
            subtitle="No unscheduled tasks! All your items have assigned due dates."
          />
        )}
      </div>
